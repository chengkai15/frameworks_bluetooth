...
static int mouse_test_cmd(void* handle, int argc, char* argv[])
{
    ...
    if (argc < 3) return CMD_PARAM_NOT_ENOUGH;
    if (bt_addr_str2ba(argv[0], &addr) < 0) return CMD_INVALID_ADDR;
    ...
    for (i = 0; i < count; i++) {
        ...
        if (bt_hid_device_send_report(handle, &addr, ms_rpt_id,
                                      (uint8_t*)rpt_data, sizeof(rpt_data)) != BT_STATUS_SUCCESS) {
    ...
    elapsed_ms = (uint32_t)((end.tv_sec - start.tv_sec) * 1000 +
